 ...
 *
 * @uses blogood_header_style()
 */
function blogood_custom_header_setup() {
	add_theme_support( 'custom-header', 
		apply_filters( 'blogood_custom_header_args', 
			array(
				...
				'height'                 => 528,
				'flex-height'            => true,
				'wp-head-callback'       => 'blogood_header_style',
			)
		)
	);
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'blogood' ),
		),
	) );
}
add_action( 'after_setup_theme', 'blogood_custom_header_setup' );

if ( ! function_exists( 'blogood_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see blogood_custom_header_setup().
	 */
	function blogood_header_style() {
		...
		wp_add_inline_style( 'blogood-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'blogood_header_style', 10 );